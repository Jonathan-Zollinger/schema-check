...
public class EncTool {
    ...
    private byte[] encrypt(byte[] inpBytes, SecretKey key) throws IdMUnitException {
        try {
    ...
    private byte[] decrypt(byte[] inpBytes, SecretKey key) throws IdMUnitException {
        try {
            Cipher cipher = Cipher.getInstance(xform);
            IvParameterSpec ips = new IvParameterSpec(iv);
    ...
    public String encryptCredentials(String password) throws IdMUnitException {
        try {
            ...
            // Encrypt
            byte[] enc = encrypt(utf8, secretKey);
    ...
    public String decryptCredentials(String password) throws IdMUnitException {
        try {
            ...
            byte[] dec = Base64.getDecoder().decode(password);

            // Decrypt
            byte[] utf8 = decrypt(dec, secretKey);
